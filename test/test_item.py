...
# ① importを削除（pytestが conftest.py を自動で読む）


# ② session_fixture → client_fixture に修正
...
def test_crate(client_fixture: TestClient):
    ...
    response = client_fixture.get('/items')
    assert item(len(response.json())) == 3
...
def test_update_異常系(client_fixture:TestClient):
    ...
    assert response.status_code == 404
    assert response.json()["details"] =="Item not updated"
